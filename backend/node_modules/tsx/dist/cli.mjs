...
`);if(D.postprocess){let{postprocess:l}=D;h=h.map((p,C)=>l.call(D,p,C))}return D.paddingTop&&h.unshift(...ju(D.paddingTop)),D.paddingBottom&&h.push(...ju(D.paddingBottom)),h.length>r&&(r=h.length),pt(Ue({},D),{lines:h})}),o=[];for(let D=0;D<r;D+=1){let c=i.map(f=>{var h;let l=(h=f.lines[D])!=null?h:"",p=Number.isFinite(f.width)?" ".repeat(f.width-De(l)):"",C=f.paddingLeftString;return f.align==="right"&&(C+=p),C+=l,f.align==="left"&&(C+=p),C+f.paddingRightString}).join("");o.push(c)}u.push(o.join(`
`))}return u.join(`
...
`));const e={eval:{type:String,alias:"e"},print:{type:String,alias:"p"}},{_:u,flags:s}=zu({flags:{...e,inputType:String,test:Boolean},help:!1,ignoreArgv:lt(!1)}),n=wn({...vn,...e}),i=["print","eval"].find(c=>!!s[c]);if(i){const{inputType:c}=s,f=s[i],h=xn(f,{loader:"default",sourcefile:"/eval.ts",format:c==="module"?"esm":"cjs"});n.unshift(`--${i}`,h.code)}wu(Mn)&&s.test&&u.length===0&&n.push("**/{test,test/**/*,test-*,*[.-_]test}.?(c|m)@(t|j)s");const o=await Rn(),D=as(n,{noCache:!!t.flags.noCache,tsconfigPath:t.flags.tsconfig});Bl(D,o),process.send&&D.on("message",c=>{process.send(c)}),D.send&&process.on("message",c=>{D.send(c)}),D.on("close",c=>{c===null&&(c=ht.signals[D.signalCode]+128),process.exit(c)})});
